      ...
      <div className = {classes.headerNav}>
        {localStorage.getItem('token')  ? (
           <div className = {classes.authContaner}> 
              ...
              <p><NavLink className = {classes.authUserName} to = {'/profile'} activeClassName={classes.activeUsername} exact>{auth.user.username}</NavLink></p>
              <img className = {classes.authImg} src={auth.user.image ? `${auth.user.image}` : "https://api.realworld.io/images/smiley-cyrus.jpeg"}  alt="avatar" />
              <button className = {classes.logOut} onClick = {onLogout}>Log Out</button>
          </div>
        ) : (
          <>
            <li><NavLink className = {classes.navSignIn}  to = {'/sign-in'} activeClassName={classes.activeSignin} exact>Sign in</NavLink></li>
